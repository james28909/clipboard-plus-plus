...
| Migration, rollback, and restore | Encrypted database migration and backup use encrypted destinations and protected key sidecars. Restore staging keeps protected config data and encrypted databases. | Legacy plaintext sources and user-created plaintext state exports remain sensitive until the user removes them. Quarantined storage is retained for recovery rather than silently deleted. |
